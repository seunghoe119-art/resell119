...
Instructions (지침):
- 확인 질문/추가 질문 금지. 한 번에 완성해.
- 모든 판매글 본문은 ✔ 기호를 사용하여 항목별로 정리합니다.
- Markdown 리스트(•, -, *) 또는 번호(1., 2.)는 사용하지 않습니다.
- 일반 게시판 스타일로 작성하며, 문단은 짧게 유지하고 가독성 있게 구성합니다.
- 사용자가 입력한 제품명과 간략한 설명을 바탕으로 중고 판매용 문구를 생성할 것, **제품의 공식 스펙, 주요 기능, 한국 시장 출시 가격 등 부족한 정보는 검색하거나 당신의 방대한 지식을 활용해 추론하고 보충하여** 글을 작성합니다. (이 부분이 핵심 수정 사항입니다)
...
Constraints(제약사항):
- **가장 중요한 규칙: 사용자에게 추가 정보를 요청하거나 질문하지 마세요. 불완전한 정보라도 당신의 지식을 총동원하여 완전한 게시글 초안을 만들어야 합니다.** (이 부분이 핵심 추가 사항입니다)
- 입력에 없는 정보는 다음 우선순위로 처리:
1) [SEARCH_CONTEXT]에 제공된 검색 요약이 있으면 그것만 사용
2) 없으면 추정/창작 금지, 안전한 기본 문구로 대체
...
위 내용을 바탕으로 중고 판매 게시글을 작성해주세요. 제품의 한국 발매가도 검색해서 포함해주세요.`;
    ...
    브랜드: ${input.brand ?? '미입력'}
    요약: ${input.briefDescription ?? '미입력'}

    [SEARCH_CONTEXT]
    - 제품 개요: <검색요약 또는 없음>
    - 한국 발매가(있으면): <예: 129~139만원, 출시가 기준 / 없으면: 없음>
    - 참고 출처: <도메인 1~2개 요약 or 없음>

    위 내용을 반영하여 중고 판매 게시글을 즉시 완성해줘.
    확인 질문 없이 곧바로 결과만 출력해.
